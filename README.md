...
- **上流（オリジナル）との同期:** GitHub Actions の **Sync from upstream**（`.github/workflows/sync-from-upstream.yml`）で、[pascalorg/editor](https://github.com/pascalorg/editor) の `main` をこのフォークの `main` にマージできます。手動実行はリポジトリの **Actions** タブから。**Mac では `mac-launcher/Sync upstream on GitHub.app`** をダブルクリックすると同じワークフローを `gh workflow run` でキューに入れられます（[GitHub CLI](https://cli.github.com/) と `gh auth login` が必要。詳細は `mac-launcher/README.md`）。マージに失敗したときは **Issue が自動作成**されます（コンフリクト対応のきっかけ用）。定期実行にしたい場合はワークフロー内の `schedule` のコメントを外してください。なお、Mac の **Update Pascal Editor** は **GitHub 上のこのフォーク** に対する `git pull` のみなので、上流の新着をローカルに反映する流れは **Sync upstream on GitHub →（Actions が緑になったら）Update Pascal Editor**、またはこの同期を Actions で実行してから Update、もしくは手動で `git fetch upstream && git merge upstream/main` してください。
